from utils.putih import color_histogram_feature_extraction
from utils.putih import knn_classifier
import os
...
def color_recognition(crop_img):
    ...
    open(current_path + '/utils/putih/' + 'test.data', 'w')
    ...
    prediction = knn_classifier.main(current_path
            + '/utils/putih/' + 'training.data',
            current_path + '/utils/putih/'
            + 'test.data')
