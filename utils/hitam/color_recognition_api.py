from utils.hitam import color_histogram_feature_extraction
from utils.hitam import knn_classifier
import os
...
def color_recognition(crop_img):
    ...
    open(current_path + '/utils/hitam/' + 'test.data', 'w')
    ...
    prediction = knn_classifier.main(current_path
            + '/utils/hitam/' + 'training.data',
            current_path + '/utils/hitam/'
            + 'test.data')
