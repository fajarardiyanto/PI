...
def encode_image_array_as_png_str(image):
  ...
  output.close()
  return png_string
def draw_bounding_box_on_image_array(current_frame_number, image,ymin,xmin,ymax,xmax,color='red',
...
def draw_bounding_box_on_image(current_frame_number,image,ymin,xmin,ymax,xmax,color='red',
                               thickness=4,display_str_list=(),use_normalized_coordinates=True):
  csv_line = "" 
  update_csv = False
  is_vehicle_detected = [0]
...
def draw_bounding_boxes_on_image_tensors(images,boxes,classes,scores,category_index,max_boxes_to_draw=20,
                                         min_score_thresh=0.2):
  visualize_boxes_fn = functools.partial(
      visualize_boxes_and_labels_on_image_array,category_index=category_index,instance_masks=None,
      keypoints=None,use_normalized_coordinates=True,max_boxes_to_draw=max_boxes_to_draw,
  ...
  images = tf.map_fn(draw_boxes, (images, boxes, classes, scores),dtype=tf.uint8,back_prop=False)
  return images
...
def visualize_boxes_and_labels_on_image_array(current_frame_number,image,boxes,classes,scores,
  ...
  for box, color in box_to_color_map.items():
    ...
    if (("mobil" in display_str_list[0]) or ("truk" in display_str_list[0]) or ("motor" in display_str_list[0])):
            is_vehicle_detected, csv_line, update_csv = draw_bounding_box_on_image_array(current_frame_number,
                ...
                display_str_list=box_to_display_str_map[box],use_normalized_coordinates=use_normalized_coordinates) 
            if keypoints is not None:
  ...
  return counter_black, csv_line_util
